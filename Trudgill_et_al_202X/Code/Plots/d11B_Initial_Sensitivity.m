...
starting_d11B4 = 8:0.2:14;
starting_co2s = BuCC.d11BCO2().create(numel(starting_d11B4));
starting_co2s.species_calibration.d11B_measured.assignToEach("value",starting_d11B4);
starting_co2s.collate("boron").assignToAll("d11B_sw",Geochemistry_Helpers.delta("B",33.32));
starting_co2s.collate("boron").collate("pH").assignToAll("pValue",NaN);
...
starting_pH = starting_co2s.collate("boron").collate("pH").collate("pValue");

d11B_4_change = [1,2,4,3.5];
final_d11B4s = Geochemistry_Helpers.delta().create([numel(starting_d11B4),numel(d11B_4_change)]);
final_d11B4s.assignToAll("standard","B");
final_d11B4s.assignToEach("value",starting_d11B4'-d11B_4_change);

final_co2s = BuCC.d11BCO2().create([numel(starting_d11B4),numel(d11B_4_change)]);
...
for change_index = 1:size(pH_change,2)-1
    plot(starting_d11B4,pH_change(:,change_index),'Color',pH_colours{change_index},'LineWidth',2);
end
plot(starting_d11B4,pH_change(:,end),'k:','LineWidth',2);

set(gca,'XTick',[]);
ylabel("\DeltapH");

xlim([min(starting_d11B4),max(starting_d11B4)]);
% ylim([-1.5,0]);
...
hold on
for change_index = 1:size(pH_change,2)-1
    plot(starting_d11B4,co2_change(:,change_index),'Color',pH_colours{change_index},'LineWidth',2);
end
plot(starting_d11B4,co2_change(:,end),'k:','LineWidth',2);

xlabel("Initial \delta^{11}B_4");
