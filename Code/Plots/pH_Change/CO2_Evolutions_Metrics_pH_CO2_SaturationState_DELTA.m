...
% legend_handle_1 = legend([high_initial_co2.handles{1}],["pH_i > 0"],'Location','SouthWest');
...
high_initial_co2.co2_quantiles(end,:) = 1e-12;
low_initial_co2.co2_quantiles(end,:) = 1e-12;

subplot_handles(2) = subplot(3,1,2);
hold on
patch([interpolation_ages,fliplr(interpolation_ages)],[high_initial_co2.co2_quantiles(:,3);flipud(high_initial_co2.co2_quantiles(:,1))],high_initial_co2.colour,'FaceAlpha',0.2,'EdgeColor','None');
...
% patch([interpolation_ages,fliplr(interpolation_ages)],[low_initial_co2.co2_quantiles(:,3);flipud(low_initial_co2.co2_quantiles(:,1))],low_initial_co2.colour,'FaceAlpha',0.2,'EdgeColor','None');

plot(interpolation_ages,high_initial_co2.co2_quantiles(:,2),'-','Color',high_initial_co2.colour,'MarkerSize',marker_size,'LineWidth',line_width,'MarkerFaceColor',medium_initial_co2.colour);
...
% plot(interpolation_ages,low_initial_co2.co2_quantiles(:,2),'-','Color',low_initial_co2.colour,'MarkerSize',marker_size,'LineWidth',line_width,'MarkerFaceColor',low_initial_co2.colour);
...
ylim([-1,6]);
current_axis = gca;
% current_axis.YAxis.MinorTickValues = log2(400*2.^[-8:8]);
% y_tick_locations = 400*2.^[-8:8];
...
ylim([-8,8]);
set(gca,'XDir','Reverse','YTick',[-8:2:8],'YTickLabels',num2str([-8:2:8]'));
